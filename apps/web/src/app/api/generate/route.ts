...
// Verbose flow logging only when explicitly enabled.
...
// Best-effort per-IP rate limit (in-memory, per lambda instance). A real
// player makes one generate call per finished quiz.
const buckets = new Map<string, { count: number; resetAt: number }>();
const RATE_MAX = 12;
const RATE_WINDOW_MS = 10 * 60 * 1000;

const rateLimited = (request: Request): boolean => {
  const ip =
...
// Cap everything user-controlled that flows into the prompt.
  ...
  try {
    ...
    if (rateLimited(request)) {
      return NextResponse.json(
        { error: "Too many reveals — let the eggnog settle." },
        { status: 429 },
      );
    }
    ...
    // Clamp user-controlled input before it reaches the prompt.
    if (body) {
      body.name = clip(body.name, 60);
      body.answers = (Array.isArray(body.answers) ? body.answers : [])
        .slice(0, 10)
        .map((a) => ({
          ...a,
          selectedOptionText: clip(a?.selectedOptionText, 240) || undefined,
          customText: clip(a?.customText, 240) || undefined,
        }));
      body.questions = (Array.isArray(body.questions) ? body.questions : [])
        .slice(0, 10)
        .map((q) => ({ ...q, prompt: clip(q?.prompt, 300) }));
    }
    ...
    if (!apiKey) {
      debugLog(`[API] ========================================`);
      debugLog(`[API] OPENAI_API_KEY not configured - using fallback`);
      debugLog(`[API] Answer seed: ${answerSeed.substring(0, 50)}...`);
      ...
      debugLog(`[API] Fallback character selected: "${fallbackChar}"`);
      debugLog(`[API] ========================================`);
      return NextResponse.json({
    ...
    if (!body?.name || !body?.questions?.length) {
      ...
      debugLog(`[API] Body received:`, { name: body?.name, questionsLength: body?.questions?.length, answersLength: body?.answers?.length });
      const availableChars = getAvailableCharacters();
      const fallbackChar = availableChars.length > 0 
        ? selectBalancedCharacter(answerSeed, availableChars)
        : getFallbackResult(answerSeed).characterName;
    ...
    debugLog(`[API] ========================================`);
    debugLog(`[API] Analyzing ${body.name} with ${body.answers.length} answers`);
    debugLog(`[API] OpenAI API key present: ${apiKey ? 'YES' : 'NO'}`);
    debugLog(`[API] Using AI: ${apiKey ? 'YES' : 'NO (fallback mode)'}`);
    ...
    debugLog(`[API] Available characters: ${availableCharacters.length}/${CHARACTERS.length}, Overused: ${overusedCharacters.length}`);
    if (overusedCharacters.length > 0) {
      debugLog(`[API] Overused characters: ${overusedCharacters.join(', ')}`);
    }
...
Return ONLY JSON: 
{
  ...
  "tagline": "optional punchy phrase"
    ...
    debugLog(`[API] --- AI Request Payload ---`);
    debugLog(`[API] Model: ${aiRequestPayload.model}`);
    debugLog(`[API] Temperature: ${aiRequestPayload.temperature}`);
    debugLog(`[API] User message length: ${aiRequestPayload.messages[1].content.length} chars`);
    debugLog(`[API] Number of answers: ${body!.answers.length}`);
    debugLog(`[API] Answers summary:`, body!.answers.map((a, i) => ({
    ...
    debugLog(`[API] --- AI API Response ---`);
    debugLog(`[API] Status: ${response.status} OK`);
    const data = await response.json();
    debugLog(`[API] Full AI response:`, JSON.stringify(data, null, 2));
    const content = data?.choices?.[0]?.message?.content ?? "";
    debugLog(`[API] Extracted content length: ${content.length} chars`);
    debugLog(`[API] Raw content:`, content.substring(0, 500) + (content.length > 500 ? '...' : ''));
    const parsed = extractJson(content);
    debugLog(`[API] Parsed JSON:`, parsed);
    ...
    debugLog(`[API] --- AI Selection Valid ---`);
    debugLog(`[API] AI selected character: "${parsed.characterName}"`);
    debugLog(`[API] AI reveal text: "${parsed.revealText.substring(0, 100)}..."`);
    if (parsed.tagline) {
      debugLog(`[API] AI tagline: "${parsed.tagline}"`);
    }
    ...
    if (recentCount >= MAX_REPEATS_IN_WINDOW) {
      ...
      debugLog(`[API] --- Character Overused, Substituting ---`);
      debugLog(`[API] AI selected: "${parsed.characterName}" (used ${recentCount} times in last ${MAX_RECENT_TRACK} selections)`);
      debugLog(`[API] Substituting with balanced selection: "${balancedChar}"`);
...
Reference at least 2 specific choices. Return JSON: {"revealText": "...", "tagline": "optional phrase"}`,
        ...
        debugLog(`[API] Could not generate reveal text for substituted character, using original`);
      }
    } else {
      debugLog(`[API] AI selection is acceptable (used ${recentCount} times in last ${MAX_RECENT_TRACK} selections)`);
    }
    ...
    if ((isKrampus && !darkSignals) || (isKevin && !kevinSignals)) {
      ...
      debugLog(`[API] --- Guardrail Substitution ---`);
      debugLog(`[API] Blocked character: "${finalCharacter}" (signals missing)`);
      debugLog(`[API] Substituting with balanced selection: "${balancedChar}"`);
      finalCharacter = balancedChar;
    }
    ...
    debugLog(`[API] --- Final Result ---`);
    debugLog(`[API] Final character: "${finalCharacter}"`);
    debugLog(`[API] Reveal text length: ${finalRevealText.length} chars`);
    debugLog(`[API] Success: Matched ${body!.name} to ${finalCharacter}`);
    debugLog(`[API] ========================================`);
